...
class YSE_Forced_Pos:
    ...
    def forcedphot_request(self,obs_data_dict,width=300,height=300,skycelldict=None,submitrequest=True):
        """
        ...
        count = 0
        for snid,ra,dec,camera,diff_id in obs_table['transient_id','ra','dec','camera','diff_id']:
        ...
        for snid,ra,dec,camera,stack_id in obs_table['transient_id','ra','dec','camera','stack_id']:
        ...
        if submitrequest:
            ...
            stamp_request = 'YSE-stamp.%i'%(time.time())
            hdr['REQ_NAME'] = stamp_request
        ...
        for snid in transient_unq:
            ...
            for ra,dec,mjd,filt,camera,diff_id, in obs_table['ra','dec','obs_mjd','photometric_band','camera','diff_id'][id_match]:
                if diff_id is not None and diff_id != 'NULL':
                    diff_data.add_row(('forcedphot_ysebot_{}'.format(diff_count),camera,ra,dec,ra,dec,filt,mjd,diff_id,skycelldict[snid]))
                    diff_count += 1
        ...
        return stamp_request,request_names,skycelldict,obs_table,0         
    ...
    def request_templates(self,trans_data,width=300,height=300,skycelldict=None):
        ...
        if skycelldict is None:
            skycelldict = {}
            for snid,ra,dec in trans_data['stack_id','ra','dec'][idx]:
                skycelldict[snid] = 'skycell.'+getskycell(ra,dec)[0]

        count = 0
        for snid,ra,dec,camera,stack_id in trans_data['transient_id','ra','dec','camera','stack_id']:
            if stack_id is None: continue
            skycell_str = skycelldict[snid]
            data.add_row((count,camera,'null','null','stamp',2049,'byid','stack',stack_id,'RINGS.V3',
                          skycell_str,2,ra,dec,width,height,'null','null',0,0,'null',0,0,'stack.for.%s'%snid) )
               ...
               'PSTAMP_NO_IMAGE_MATCH' not in mdc_stamps[k]['ERROR_STR']:
                ...
                if 'NO_VALID_PIXELS' not in mdc_stamps[k]['ERROR_STR'] and 'PSTAMP_NO_IMAGE_MATCH' not in mdc_stamps[k]['ERROR_STR']:
                    if img_type == 'stack' and img_id not in image_dict[transient]['stack_image_id']:
                        ...
                        image_dict[transient]['ra'] += [coord_data['ra'][k]]
                        image_dict[transient]['dec'] += [coord_data['dec'][k]]
    ...
    def get_gal_flux(self,img_dict):
        ...
        for k in img_dict.keys():
            ...
            for i in range(len(img_dict[k]['stack_image_link'])):
                ...
                outdir = "%s/%s/%i"%(basedir,k,int(img_dict[k]['stack_image_mjd'][i]))
                ...
                img_dict[k]['gal_flux'] += [factor * apstats.sum]
                img_dict[k]['gal_err'] += [factor * apstats.std]

                if self.options.nofitsdownload:
                     shutil.rmtree(outdir)
                else:
                    fits_to_png(ff,outfile.replace('fits','png'))
                    img_dict[k]['stack_file'] += [outfile.replace('{}/'.format(basedir),'')]
                    img_dict[k]['stack_file'+'_png'] += [outfile.replace('{}/'.format(basedir),'').replace('.fits','.png')]
...
#    def get_all_images(self,img_dict):
#
...
""")
        ...
        stamp_request_name,phot_request_names,skycelldict,obs_table,status = self.forcedphot_request(obs_data_dict)
        ...
        jobs_done = False
        tstart = time.time()
        while not jobs_done and time.time()-tstart < max_time*60:
            ...
            for phot_request_name in phot_request_names:
                ...
                doneall_phot *= done_phot
            jobs_done = done_stamp and doneall_phot
        ...
        stack_request_name,skycelldict = self.request_templates(phot_table,skycelldict=skycelldict)
        ...
        stack_img_dict = self.get_stamps(stack_request_name, phot_table['ra','dec'])
