...
class YSE_Forced_Pos:
    ...
    def forcedphot_request(self,obs_data_dict,skycelldict):
        ...
        for snid_unq in np.unique(transient_list):
            ...
            for snid,ra,dec,mjd,filt,camera,diff_id,warp_id in \
                zip(transient_list[transient_list == snid_unq],ra_list[transient_list == snid_unq],dec_list[transient_list == snid_unq],
                    mjd_list[transient_list == snid_unq],filt_list[transient_list == snid_unq],camera_list[transient_list == snid_unq],
                    diff_id_list[transient_list == snid_unq], warp_id_list[transient_list == snid_unq]):
                if diff_id is not None and diff_id != 'NULL':
                     ...
                     count += 1
                if warp_id is not None and diff_id != 'NULL':
                    data.add_row(('forcedphot_ysebot_{}'.format(count),camera,ra,dec,ra,dec,filt,mjd,warp_id,skycelldict[snid_unq]))
                    count += 1
